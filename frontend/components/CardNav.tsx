    ...
    >
      ...
      >
        <div className="card-nav-top absolute inset-x-0 top-0 h-[60px] flex items-center justify-between p-2 pl-[1.1rem] z-[2]">
          ...
          <Link
            href={ctaHref}
            className="hidden md:inline-flex border-0 rounded-lg px-4 h-full font-medium cursor-pointer transition-colors duration-300 items-center"
        ...
        >
          {(items || []).slice(0, 3).map((item, idx) => (
            <div
              key={`${item.label}-${idx}`}
              className="select-none relative flex flex-col gap-2 p-[12px_16px] rounded-lg min-w-0 flex-[1_1_auto] h-auto min-h-[60px] md:h-full md:min-h-0 md:flex-[1_1_0%]"
              ref={setCardRef(idx)}
              style={{ backgroundColor: item.bgColor ?? "#0D0716", color: item.textColor ?? "#fff" }}
            >
              <div className="font-medium tracking-[-0.5px] text-[18px] md:text-[22px]">
                {item.label}
              </div>
              <div className="mt-auto flex flex-col gap-[2px]">
                {item.links?.map((lnk, i) => {
                  const link = (
                    <span className="inline-flex items-center gap-[6px] no-underline cursor-pointer transition-opacity duration-300 hover:opacity-75 text-[15px] md:text-[16px]">
